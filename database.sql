...
-- SOFT DELETE
-- 1. Criar uma coluna na table products chamada "deleted_at" (desativar)
ALTER TABLE products ADD COLUMN "deleted_at" timestamp;
-- 2. Criar uma Regra que vai rodar todas as vezes que solicitar o DELETE
CREATE OR REPLACE RULE delete_product AS
ON DELETE TO products DO INSTEAD
UPDATE products
SET deleted_at =  now()
WHERE products.id = old.id;
-- 3. Criar uma VIEW para puxar apenas dados ativos
